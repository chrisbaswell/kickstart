    ...
    @endif
            <button
                ...
                {{ $attributes->merge($getExtraAttributes())->class([
                    'relative inline-flex shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:border-primary-600 focus:ring-2 focus:ring-primary-600 bg-base-200',
                    'border-base-300' => ! $errors->has($getStatePath()),
                    'border-danger-300 ring-danger-600' => $errors->has($getStatePath()),
                ]) }}
            ...
            >
                ...
                >
                    ...
                    >
                        @if ($hasOffIcon())
                            <x-dynamic-component :component="$getOffIcon()" class="bg-white h-3 w-3 text-base-400" />
                        @endif
                    </span>
