    ...
    >
        ...
        >
            <input
                ...
                x-model="displayText"
                {!! ($id = $getId()) ? "id=\"{$id}\"" : null !!}
                class="w-full h-full p-0 placeholder-base-400 border-0 focus:placeholder-base-500 focus:ring-0 focus:outline-none"
            />

            <span class="absolute inset-y-0 right-0 rtl:right-auto rtl:left-0 flex items-center pr-2 rtl:pl-2 pointer-events-none">
                <svg class="w-5 h-5 text-base-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    @if ($hasDate())
                        <div class="flex items-center justify-between space-x-1 rtl:space-x-reverse">
                            <select
                                ...
                                class="grow p-0 text-lg font-medium text-base-800 border-0 cursor-pointer focus:ring-0 focus:outline-none"
                            >
                        ...
                        <div class="grid grid-cols-7 gap-1">
                            <template x-for="(day, index) in getDayLabels()" :key="index">
                                <div
                                    x-text="day"
                                    class="text-xs font-medium text-center text-base-800"
                                ></div>
                            </template>
                        </div>
                        ...
                        <div role="grid" class="grid grid-cols-7 gap-1">
                            ...
                            <template x-for="day in daysInFocusedMonth" x-bind:key="day">
                                <div
                                    ...
                                    x-bind:class="{
                                        'text-base-700': ! dayIsSelected(day),
                    ...
                    @if ($hasTime())
                        ...
                        >
                            <input
                                ...
                                @class([
                                    'w-16 p-0 pr-1 text-xl text-center text-base-700 border-0 focus:ring-0 focus:outline-none',
                                    'bg-base-50' => $hasDate(),
                                ])
                            />
                            ...
                            <span
                                @class([
                                    'text-xl font-medium text-base-700',
                                    'bg-base-50' => $hasDate(),
                                ])
                            >:</span>
